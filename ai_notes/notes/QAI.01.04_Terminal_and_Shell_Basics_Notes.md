...
### 16.6 Remove only a verified empty practice directory

**Remove directory** means delete a folder. Practice this only with a **new, empty** `remove_me` folder inside the disposable `terminal-lab`; never point the command at `notes`, `archive`, `code-lab`, or their parent folders. In PowerShell, preview before removal:

```powershell
Get-Location
New-Item -ItemType Directory -Path .\remove_me
Get-ChildItem -LiteralPath .\remove_me
Remove-Item -LiteralPath .\remove_me -WhatIf
Test-Path -LiteralPath .\remove_me
Remove-Item -LiteralPath .\remove_me
Test-Path -LiteralPath .\remove_me
```

**Expected:** the folder listing is empty. `-WhatIf` reports the proposed operation **without performing it**; the first `Test-Path` prints `True`. The later `Remove-Item` removes the confirmed empty folder; the second `Test-Path` prints `False`. Do not add `-Recurse` or `-Force` to make a failed deletion succeed. If the folder is not empty or the current directory is wrong, stop and inspect the target.

In Bash/Zsh, from the same disposable `terminal-lab` folder, create and remove a newly empty folder with `rmdir`, which refuses a nonempty directory:
...
rmdir remove_me
test ! -e remove_me
echo $?  # 0 means the folder no longer exists
```

This demonstration completes the file-action vocabulary from `QAI.01.03`: a directory removal has a real file-system effect, while clearing the terminal display does not. Recovery of deleted folders is not promised; the lab removes only a disposable empty one.
