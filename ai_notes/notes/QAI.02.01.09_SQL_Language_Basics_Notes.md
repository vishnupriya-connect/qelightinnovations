...
| `courses`, `course_id`, `course_title`, `label` | **Identifiers**: names of the table, columns, and a result-column alias | They identify an object or output name; `label` is only the output heading |
| `course_title AS label` | Part of a **SQL expression** in the select list; its value comes from each row's `course_title` | The database evaluates it for each selected row |
| `SELECT course_id, course_title AS label` | A **SELECT clause**: specifies result columns | Begins with `SELECT` |
| `FROM courses` | A **FROM clause**: identifies the source table | Begins with `FROM` |
...
A **SQL statement** is a complete request, such as the whole `SELECT ... FROM ...;`. A **SQL query** is a statement, or a part of one, that asks for data. This `SELECT` is both a statement and a query. `FROM courses` by itself is only a clause, not a complete statement. SQL keywords are conventionally capitalised for readability; `SELECT` and `select` are both recognised by SQLite. Identifier spelling still matters: `course_title` is a different name from `course_name`.
